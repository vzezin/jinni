...
The intent-shaped brief: *"Objective: nightly CI is back under ~15 min, or we know precisely why it can't be. Because: the team ships against this build; every extra 10 min moves the deploy window past standup, so a diagnosis without a fix still pays. It jumped 12→40 min somewhere in the last two weeks (hypothesis: coincides with the monorepo split, unverified). Floors: don't touch prod deploy jobs — same pipeline, blast radius (if the cause lives there, stop and report); no history rewrites — the history is the evidence we fold; if a fix seems to need one, stop and report. Free: methods, tooling, cache surgery on CI-local state, bisecting the config. Accept, judged by me after you return: three consecutive green runs at the claimed duration, with the pipeline still doing everything it did before, plus the causal story with evidence. If the ground contradicts any of this — the slowdown began before the monorepo split, or the cause is upstream of us — stop and bring the finding; that's a success. If two parts of this brief pull apart, take the reading whose because holds, say which, and go; contradictions here are my errors, not tests. Return: what you changed, what you ruled out, what you left standing. Budget: this afternoon, helpers fine; at 6 pm local time stop, log your position and the considerations needed to restart into a file, and report where you are."*

Do not blindly copy the second brief: rethink the wording with each task, every time.

**The channel the procedure doesn't show.** Most orders are a sentence in conversation, not a brief, and our one loss at a gate came there, through a casual order the jinni read wide. Every sentence the jinni shall act on is contract text: the first forms the contract, each later one amends it. Grade each as direction, support, leaning, or question, so the jinni does not invent a leaning you never had; an amendment says which clauses it displaces and which stand, or the jinni merges the texts by its own priors. Before a casual order touches a gated or irreversible surface, say its objective and floor in one line, or you get consequences instead of a question. In depth the same: a brief sits over a skill, a harness sheet and a system prompt, and amends them all; and every slightest contradiction without assigned precedence is a fork. Role labels like system, user, tool are a few tiers against the whole text stream, and even frontier models resolve written tiers poorly. Put the contradiction management clause into the system prompt before the fork arrives.
...
- [ ] Acceptance test stated — and I could run it myself right now; if I can't, back to §8.1 (§6.4)
